...
- проверенный implementation commit: `9aeb897`;
- baseline `cargo test`: PASS — 788 unit tests, 15 ignored; integration и doc tests также завершились без ошибок;
- итоговый `cargo test`: exit 0 — 823 unit tests passed, 15 ignored; по всем cargo test binaries суммарно 990 passed, 44 ignored, 0 failed;
...
| R-07 | overlapping/adjacent/self-overlapping literal и regex ranges — `merges_overlapping_and_adjacent_ranges_once`, `merges_self_overlapping_literal_matches`, `zero_width_regex_terminates_and_preserves_remaining_text` | `cargo test` | PASS |
...
| R-15 | Docker health/tail/error daemon diagnostics — `health_diagnostic_omits_probe_output`, `container_tail_summary_discards_payload_bytes`, `docker_error_diagnostic_discards_daemon_payload`, Docker trace canary tests | `cargo test` | PASS |
