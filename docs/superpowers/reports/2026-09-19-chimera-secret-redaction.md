...
- проверенный implementation commit: `fa9902be00673fbcbe59a3089befde5166dbf12c`;
- baseline `cargo test`: PASS — 788 unit tests, 15 ignored; integration и doc tests также завершились без ошибок;
- итоговый `cargo test --quiet`: exit 0 — 820 unit tests passed, 15 ignored; по всем cargo test binaries суммарно 987 passed, 44 ignored, 0 failed;
...
| R-04 | URI/XML/Base64/expression/command-line variants — `masks_supported_encoded_forms_with_literal_expectations` | `cargo test` | PASS |
| R-05 | manifest regex hint — `masks_regex_hint_and_literal_pattern` | `cargo test` | PASS |
| R-06 | invalid/unsupported hint без echo payload — `rejects_invalid_regex_without_echoing_type_or_pattern`, `unsupported_hint_reports_only_its_index` | `cargo test` | PASS |
| R-07 | overlapping/adjacent literal и regex ranges — `merges_overlapping_and_adjacent_ranges_once`, `zero_width_regex_terminates_and_preserves_remaining_text` | `cargo test` | PASS |
...
| R-15 | Docker health/tail daemon diagnostics — `health_diagnostic_omits_probe_output`, `container_tail_summary_discards_payload_bytes` | `cargo test docker::resources::resources_test -- --nocapture` | PASS |
