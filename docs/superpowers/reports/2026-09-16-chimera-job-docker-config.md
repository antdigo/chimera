...
The static, non-Docker, and ignored Docker evidence below is green. On
2026-09-17, the ignored suite ran against an isolated rootless Docker-in-Docker
daemon whose `SecurityOptions` included `name=rootless`. The C-10 BuildKit,
registry, HTTPD, Rust, Node, and Docker CLI dependencies were resolved by immutable
digest and, where needed by the nested daemon, preloaded; the three public Docker
actions were downloaded only at their specified commit pins.
`cargo test --offline -- --ignored` passed all 20 ignored tests with
zero failures, including C-03 and C-10.
...
| C-03 | Passed in rootless Docker: `tests/job_docker_config_docker_test.rs::concurrent_logout_does_not_remove_other_job_credentials` (`#[ignore]`) proves that logout A does not remove B's credentials and B can perform a subsequent authenticated registry operation. |
...
| C-10 | Passed in rootless Docker/Buildx: `tests/job_docker_config_docker_test.rs::pinned_buildx_flow_uses_job_config_and_original_socket` (`#[ignore]`) completed pinned setup → login → build/push → pull → post against the local authenticated registry. Its guards require an explicit coherent rootless Unix endpoint, parse daemon security options for `name=rootless`, and reject missing, failed, cancelled, duplicated, or unrelated pinned post completion records. |
...
The Docker run used only synthetic credentials and a local registry. After the
suite, the isolated rootless daemon contained no test containers or volumes; only
its reusable base-image cache remained.
